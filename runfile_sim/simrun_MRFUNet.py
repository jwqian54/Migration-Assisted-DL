...
from model.MRFUNet import MRFUNet
...
from loaddata.matload import create_data_loaders_W_crop
...
model_type = f'2ndStage_MSELoss_2DV13_Per_DMRF_UNet_Checked_Xavier_Batch32_rawwotz_{num_test+num_train}_fold{ii_fold}'
model_folder = '../ResultsSim/'+model_type

if not os.path.exists(model_folder):
    os.makedirs(model_folder)

input_train = load_2ndStage_Migration(data_folder+matfilename_migration_train,migration_prefix,migration_suffix,index_digit,train_index,shift_aug=Data_Augmentation)

input_val = load_2ndStage_Migration(data_folder+matfilename_migration_test,migration_prefix,migration_suffix,index_digit,test_index,shift_aug=Data_Augmentation)
...
train_loader, val_loader = create_data_loaders_W_crop(input_train, output_train, input_val, output_val, crop_train, crop_val, batch_size=32)
...
training_loss_write, training_mse_write, training_ssim_write,testing_loss_write, testing_mse_write, testing_ssim_write = train_hybridloss(train_loader, val_loader, model, model_folder, total_epoch, criterion, optimizer, scheduler,PlotOption=True,FastSkipping=False)
...
def compute_metrics(output, target):
    output = output.squeeze()  # Convert to NumPy array
    target = target.squeeze()

    # Calculate SSIM using scikit-image
    ssim_value1 = ssim(target, output, data_range=output.max() - output.min())
    ssim_value2 = ssim(target, output, data_range=target.max() - target.min())

    # Calculate MAE and MSE using NumPy
    mae_value = np.abs(target - output).mean()
    mse_value = ((target - output) ** 2).mean()

    return ssim_value1,ssim_value2, mae_value, mse_value
