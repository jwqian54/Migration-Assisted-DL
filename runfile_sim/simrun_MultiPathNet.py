import os
import sys
# 将项目根 D:\tgrs_open 加入路径，以便找到 model/loaddata 等包
...
from model.MultiPathNet import MultiPathNet

from skimage.metrics import structural_similarity as ssim
from sklearn.metrics import mean_squared_error, mean_absolute_error
from loaddata.matload import load_mat_file_pytorch
from loaddata.matload import load_crop_info_byindex
from loaddata.matload import create_data_loaders_RefStack


from model_train.train_config import train_hybridloss_RefStack
import torch
...
from loaddata.matload import load_2ndStage_Migration
from loaddata.matload import load_1stStage_Bscan
...
input2name_train = "Input_Migration"
input2name_val = "Input_Migration"

input2_prefix = "entropy_twodefect_"
input2_suffix = "v"
...
model_type = f'2ndStage_MSELoss_2DV13_Per_MultiPathEncoder_Rawwotz_Checked_Xavier_lr{lr_value}_Batch32_{num_test+num_train}_fold{ii_fold}'
model_folder = '../ResultsSim/'+model_type

if not os.path.exists(model_folder):
    os.makedirs(model_folder)

input1_train = load_2ndStage_Migration(data_folder+matfilename_migration_train,migration_prefix,migration_suffix,index_digit,train_index,shift_aug=Data_Augmentation)

input1_val = load_2ndStage_Migration(data_folder+matfilename_migration_test,migration_prefix,migration_suffix,index_digit,test_index,shift_aug=Data_Augmentation)


input2_train = load_2ndStage_Migration(data_folder+input2name_train,input2_prefix,input2_suffix,index_digit,train_index,shift_aug=Data_Augmentation)

input2_val = load_2ndStage_Migration(data_folder+input2name_val,input2_prefix,input2_suffix,index_digit,test_index,shift_aug=Data_Augmentation)
...
train_loader, val_loader = create_data_loaders_RefStack(input1_train, input2_train,output_train,input1_val,input2_val,output_val, batch_size=32)
...
transfered_model_path = "D:\OneDrive - Nanyang Technological University/research\TGRS_Image\pytorch_code\Exclude_2DV13/2ndStage_MSELoss_2DV13_Per_MultiPathEncoder_Checked_Xavier_Batch32_3000_fold0/best_fold0.pth"

model = MultiPathNet(1,1)
...
if is_cuda:
    model.cuda()
model.load_state_dict(torch.load(transfered_model_path))
...
training_loss_write, training_mse_write, training_ssim_write,testing_loss_write, testing_mse_write, testing_ssim_write = train_hybridloss_RefStack(train_loader, val_loader, model, model_folder, total_epoch, criterion, optimizer, scheduler,PlotOption=True)
...
with torch.no_grad():
    ...
    for i, data in enumerate(val_loader):
        ...
        x_val, x_val2,y_true = data
        x_val = x_val.float().to('cuda')
        x_val2 = x_val2.float().to('cuda')
        y_true = y_true.float().to('cuda')

        y_pred = model_load(x_val,x_val2)


        x_val_np = x_val.cpu().numpy()
        x_val2_np = x_val2.cpu().numpy()
        ...
        for ii_sample in range(len(x_val_np)):
            ...
            output_x_val2.append(x_val2[ii_sample].squeeze())
            ...
            if i==0:
                ...
                plt.imshow(x_val2_np[ii_sample].squeeze(), cmap='gray')
...
output_x_val2_np = torch.stack(output_x_val2).cpu().numpy()
...
scio.savemat((model_folder+"/best_performance.mat"),mdict={'mse_ave': average_mse,'mse':mse_values_np, 'mae_ave':average_mae,'mae':mae_values_np ,'ssim_ave': average_ssim, 'ssim':ssim_values_np, 'y_pred':output_y_pred_np, 'y_true':output_y_true_np, 'x_val':output_x_val_np,'x_val2':output_x_val2_np})
...
with torch.no_grad():
    ...
    for i, data in enumerate(val_loader):
        ...
        for ii_sample in range(len(x_val_np)):
            ...
            mse_value = F.mse_loss(y_pred_sample, y_true_sample)
            ssim_value = ssim(y_pred_sample, y_true_sample, data_range=y_true_sample.max() - y_true_sample.min(),
...
scio.savemat((model_folder+"/epoch{}_performance.mat".format(total_epoch-1)),mdict={'mse_ave': average_mse,'mse':mse_values_np, 'mae_ave':average_mae,'mae':mae_values_np ,'ssim_ave': average_ssim, 'ssim':ssim_values_np, 'y_pred':output_y_pred_np, 'y_true':output_y_true_np, 'x_val':output_x_val_np,'x_val2':output_x_val2_np})
